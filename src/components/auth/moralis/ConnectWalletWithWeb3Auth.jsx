...
export default function ConnectWallet() {
  ...
  const loginWithConnector = async (connector, id) => {
    ...
    try {
      await authenticate({
        connector: connector,
        signingMessage: "Login to TixHive with your Coinbase Wallet",
      });
  ...
  useEffect(() => {
    ...
    if (
      source &&
      (source === "Metamask_Dapp_Link" || source === "TrustWallet_Dapp_Link") &&
      window.ethereum &&
      !user
    ) {
      autoLogin();
  ...
  const UDSignUp = async () => {
    ...
    try {
      ...
      if (connectedUser) {
        window.localStorage.setItem("connectorId", "UAuthMoralis");
      }
      return;
    } catch (e) {
      console.log(e);
    ...
    <div>
      <div className="flex items-center">
        <button
          ...
          className={`${
            showRed ? "bg-[#D30000]" : "bg-brand-blue"
                ...
                {connectors.map((connector, connectorIndex) => (
                  <div
                    ...
                    className={`${
                      ...
                      !window.ethereum && connector.title === "Coin98"
                        ? "hidden"
                        : "flex"
                    } ${
                      window.ethereum &&
                      ...
                      >
                        ...
                        <div>
                          <a
                            ...
                            rel="noreferrer"
                            href={wallet.link}
                            className="px-3 py-1 text-sm font-semibold uppercase rounded-full text-brand-blue bg-slate-200"
                          >
                            Get
                          </a>
                        </div>
                ...
                <div className="flex flex-col px-5 text-xs py-6 md:py-14 bg-slate-100 md:bg-transparent md:px-14">
                  ...
                  <div className="flex justify-center mt-10">
                    <button
                      ...
                      className="px-3 py-2 text-sm text-white rounded-full bg-brand-blue"
                    >
                      Get A Wallet
                    </button>
                  </div>
